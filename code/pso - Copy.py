...
def run_pso(clients_demands,distance_matrix,n_vehicles,
    ...
    return min_val,min_val_particle , _avgSol/n_exp , _avgTime/n_exp,ctest,coptimo,all_gb_best_vals


def run_test():
    files = [('A-n32-k5.vrp',5,True),('A-n33-k6.vrp',6,True), ('A-n44-k6.vrp',6,True), ('A-n65-k9.vrp',9,True) , ('A-n80-k10.vrp',10,True)]
    l_n_particles = [10,100,1000]#[100000]#[10,100,1000]#[10,100,1000]
    l_n_iterations = [30,100,1000]#[30]# 
        ...
        for n_iterations in l_n_iterations:
            for file,n_vehicle,_do in files:
                if _do:
                    ...
                    minval , min_val_particle, avg , avgTime ,ctest, coptimo,all_gb_best_vals = run_pso(demand_matrix,distance_matrix,n_vehicle,
                    ...
                    with open(f'V0-{file[:-4]}-SolInfo-{n_particles}-Iteraciones-{n_iterations}.txt', "w") as outfile:
...
#     _cap = 0
#     for c in t1:
#         _cap += demand_matrix[c-1]
#     cap.append(_cap)

# #print(demand_matrix[0])
# print(cap)    
...
#         if j == 0:
#             #_val += distance_matrix[0][t1[i][j]-1]
#             _x += distance_matrix[0][t[i][j]-1]
#         else:
#             #_val += distance_matrix[t1[i][j-1]-1][t1[i][j]-1]
#             _x += distance_matrix[t[i][j-1]-1][t[i][j]-1]
#             if j == len(t[i]) - 1:
#             #    _val += distance_matrix[t1[i][j]-1][0]
#                 _x += distance_matrix[t[i][j]-1][0]
...
#         if i == 0:
#             _cap += distance_matrix[0][t1[i]-1]
#         else:
#             _cap += distance_matrix[t1[i-1]-1][t1[i]-1]
#             if i == len(t1) - 1:
#                 _cap += distance_matrix[t1[i]-1][0]
#     cap.append(_cap)
...
# print(random_permutation([x for x in range(2,total_customers+1)]))
# trucks = get_sol(p,demand_matrix,capacity_of_vehicle)
# print(trucks)
    #print(eval_sol(trucks,distance_matrix))
...
#[25, 9, 12, 19, 25, 31, 27, 31, 1, 25, 1, 2, 8, 8, 2, 28, 15, 21, 6, 6, 25, 15, 31, 28, 29, 29, 19, 19, 14, 14, 14]